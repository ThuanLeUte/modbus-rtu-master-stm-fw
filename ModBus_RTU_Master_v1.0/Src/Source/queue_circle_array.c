...
{
    if(queue == 0 || array == 0 || queueSize == 0) return RET_FALSE;
...
uint8_t Queue_EnQueue(struct_queue_array *queue, queue_data_type element)
{
    ...
    {
        if (Queue_IsEmpty(queue)) queue->front = 1;        
        if (queue->rear == queue->queueSize) queue->rear = 1; else queue->rear++;
...
queue_data_type Queue_DeQueue(struct_queue_array *queue)
{
    ...
    else
    {
        ...
        {            
            if (queue->front == queue->queueSize) queue->front = 1; else queue->front++;
        }
        return retVal;
    }
}
...
uint8_t Queue_PeekIndex(struct_queue_array *queue, queue_data_type *element, uint16_t index)
{
    ...
    if (index > Queue_GetFill(queue)) return 0xFF;//Error
...
{
    if (Queue_IsEmpty(q)) return (uint16_t)q->queueSize;
